<div>
  ...
  @if ($quickAddGroup && $quickAddProductId)
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4" style="z-index: 2147483647;" wire:key="cart-quick-add-modal-{{ $quickAddProductId }}">
      ...
      <div x-data @click.stop class="relative bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6">
          <livewire:cart.quick-add-product :product="\Dashed\DashedEcommerceCore\Models\Product::find($quickAddProductId)" :key="'quick-add-cart-group-'.$quickAddGroupId" />
        </div>
      </div>
    </div>
    </template>
