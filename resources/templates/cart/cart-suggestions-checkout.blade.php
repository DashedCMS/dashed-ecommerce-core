<div class="min-w-0">
    @if ($suggestions->isNotEmpty())
        <div class="my-4 p-3 rounded-md overflow-hidden min-w-0 {{ ($progress['gap'] ?? 0) > 0 ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200' }}">
        ...
        <template x-teleport="body">
            <div class="fixed inset-0 flex items-center justify-center p-4" style="z-index: 2147483647;" wire:key="checkout-quick-add-modal-{{ $quickAddProductId }}">
                ...
                <div x-data @click.stop class="relative bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-6">
                        <livewire:cart.quick-add-product :product="\Dashed\DashedEcommerceCore\Models\Product::find($quickAddProductId)" :key="'quick-add-checkout-group-'.$quickAddGroupId" />
                    </div>
                </div>
            </div>
        </template>
