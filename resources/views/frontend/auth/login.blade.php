    ...
    <x-container>
        <div class="py-0 sm:py-8 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
            <form class="flex flex-col space-y-6" wire:submit.prevent="register">
                <h2 class="font-display font-bold text-xl">{{Translation::get('register', 'login', 'Register')}}</h2>
                ...
                <x-fields.input required type="password" model="registerPasswordConfirmation" id="password_confirmation" class="w-full" :label="Translation::get('repeat-password', 'login', 'Repeat password')" />
                <x-fields.checkbox model="registerRememberMe" id="register_remember_me" :label="Translation::get('remember-me', 'login', 'Remember me')" />

                <button class="button button-white-on-primary w-full">{{Translation::get('register', 'login', 'Register')}}</button>
            </form>
            <form wire:submit.prevent="login"
                  class="space-y-6 flex flex-col mt-auto">
                <h2 class="font-display font-bold text-xl">{{Translation::get('login', 'login', 'Login')}}</h2>
                ...
                <button class="button button-white-on-primary mt-auto">{{Translation::get('login', 'login', 'Login')}}</button>
            </form>
            <div class="lg:col-span-2 ml-auto -mt-4">
                <a class="text-primary-500"
                   href="{{AccountHelper::getForgotPasswordUrl()}}">{{Translation::get('forgot-password', 'login', 'Forgot password?')}}</a>
            </div>
        </div>
    </x-container>
</div>
