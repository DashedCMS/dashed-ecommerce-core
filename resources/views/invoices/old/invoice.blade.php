            ...
            <div class='left' style="width:100%; display:block;margin-left:5px;">
                ...
                <div style="width:31%;display:inline-block; position:relative;top:0;">
                    <p>
                        ...
                        @if($order->status == 'partially_paid')
                            <span>{{CurrencyHelper::formatPriceForPDF($order->paidAmount)}}</span><br>
                            <span>{{CurrencyHelper::formatPriceForPDF($order->openAmount)}}</span><br>
                        @endif
                    </p>
                </div>
            </div>
    ...
    <div>
        <table class="border-product">
            ...
            <tr>
                ...
                <td>
                    {{CurrencyHelper::formatPriceForPDF($order->subtotal, 'EUR', true)}}
                </td>
            </tr>
            @if(!$order->shippingMethod || !$order->shippingMethod->shippingZone->hide_vat_on_invoice)
                ...
                @if(count($order->vat_percentages ?: []) != 1)
                    <tr>
                        ...
                        <td>
                            {{CurrencyHelper::formatPriceForPDF($order->btw, 'EUR', true)}}
                        </td>
                    </tr>
                @endif
            @endif
            ...
            {{--                        {{CurrencyHelper::formatPriceForPDF($order->shipping_costs, 'EUR', true)}} @else {{Translation::get('free-shipping', 'invoice', 'Free')}} @endif--}}
            {{--                </td>--}}
            {{--            </tr>--}}
            ...
            {{--                    </td>--}}
            {{--                    <td>--}}
            {{--                        {{CurrencyHelper::formatPriceForPDF($order->payment_costs, 'EUR', true)}}--}}
            {{--                    </td>--}}
            {{--                </tr>--}}
            {{--            @endif--}}
            @if($order->discount != 0.00)
                <tr>
                    ...
                    <td>
                        {{CurrencyHelper::formatPriceForPDF($order->discount, 'EUR', true)}}
                    </td>
                </tr>
            @endif
            <tr>
                ...
                <td>
                    {{CurrencyHelper::formatPriceForPDF($order->total, 'EUR', true)}}
                </td>
            </tr>
        </table>
    </div>
