...
<div class="relative w-full h-full"
    ...
    <div class="h-full w-full flex flex-col bg-neutral-950 text-white overflow-hidden">

        {{-- ============== TOPBAR ============== --}}
        <header class="shrink-0 h-16 px-5 flex items-center justify-between border-b border-white/10 bg-white/[0.03]">
            <div class="flex items-center gap-4 min-w-0">
                ...
                <span class="ml-1 px-3 h-9 hidden sm:grid place-items-center rounded-lg bg-white/5 font-mono font-semibold tabular-nums text-lg" x-html="time"></span>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" x-cloak x-show="products.length"
                        ...
                        class="pos-tool size-10 rounded-lg bg-white/5 hover:bg-white/10 grid place-items-center transition-colors">
                ...
                <button data-emojis="🔥 💥 🎉 🥳 💸 ✨ 🚀 😎 🌈 🪩 🧨 👑 💃 🕺 🍾 🎊"
                        ...
                        class="pos-tool size-10 rounded-lg bg-white/5 hover:bg-white/10 grid place-items-center transition-colors text-lg">🥳</button>
                {{-- Winkelwagen legen --}}
                <button x-cloak x-show="products.length" @click="clearProducts" x-bind:disabled="loading"
                        title="Winkelwagen legen"
                        class="pos-tool size-10 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 grid place-items-center transition-colors">
        ...
        <div class="flex-1 min-h-0 flex">
            ...
            <section class="flex-1 min-w-0 flex flex-col p-5 gap-4">
                ...
                <form @submit.prevent="selectProduct" class="shrink-0">
                    <div class="relative">
                        ...
                        <input autofocus x-model="searchProductQuery" id="search-product-query"
                               ...
                               class="w-full h-12 rounded-xl border border-white/10 pl-11 pr-12 text-lg text-white placeholder:text-white/40 focus:ring-2 focus:ring-primary-500 outline-none transition-colors">
                ...
                <div class="shrink-0 flex flex-wrap gap-2" x-cloak x-show="!searchProductQuery">
                    ...
                    <button @click="toggle('customProductPopup')" x-bind:disabled="loading"
                            class="pos-tool h-11 px-3.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 flex items-center gap-2 text-sm font-medium transition-colors">
                    ...
                    <button @click="showOrdersPopup()" x-bind:disabled="loading"
                            class="pos-tool focus-search-order h-11 px-3.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 flex items-center gap-2 text-sm font-medium transition-colors">
                ...
                <div class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1 scrollbar-wide">
                    ...
                    <div x-cloak x-show="!loadingSearchedProducts && searchProductQuery && searchedProducts.length"
                        ...
                        <template x-for="product in searchedProducts" :key="product.id">
                            <button @click="addProduct(product.id)"
                                    class="text-left rounded-2xl p-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-primary-500 transition-all flex flex-col gap-3">
                                <div class="rounded-xl overflow-hidden bg-white/5 aspect-[4/3] grid place-items-center" x-show="product.image">
                                    <img :src="product.image" class="object-cover w-full h-full">
                                </div>
                                <div class="rounded-xl bg-white/5 aspect-[4/3] grid place-items-center text-white/20" x-show="!product.image">
                                ...
                                <div class="leading-tight">
                                    ...
                                    <p x-show="product.actual_stock < 1 && product.stock < 1" class="text-xs font-medium mt-0.5 text-red-400">uitverkocht</p>
                                </div>
                            </button>
                        </template>
                    </div>
                    {{-- Geen resultaten --}}
                    <div x-cloak x-show="!loadingSearchedProducts && searchProductQuery && !searchedProducts.length"
                         class="h-full grid place-items-center text-center text-white/40">
                        <div>
                    ...
                    {{-- Laden --}}
                    <div x-cloak x-show="loadingSearchedProducts" class="h-full grid place-items-center text-center text-white/40">
            ...
            <aside class="w-[400px] shrink-0 flex flex-col border-l border-white/10 bg-white/[0.03]">

                {{-- Kop --}}
                <div class="shrink-0 h-16 px-5 flex items-center justify-between border-b border-white/10">
                    <div class="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary-400"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12"/></svg>
                        <span class="font-bold text-lg">Bestelling</span>
                        <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-white/10 text-white/70" x-html="totalQuantity() + ' artikelen'">0 artikelen</span>
                    </div>
                    <button x-cloak x-show="products.length" @click="clearProducts" x-bind:disabled="loading"
                            class="pos-tool text-sm text-white/50 hover:text-red-400 font-medium flex items-center gap-1 transition-colors">
                ...
                <div class="flex-1 min-h-0 overflow-y-auto p-3 space-y-2 scrollbar-wide">
                    <template x-for="product in products" :key="product.identifier">
                        <div class="rounded-xl bg-white/5 border border-white/10 p-3 flex gap-3">
                            <div class="relative shrink-0">
                                ...
                                <img x-cloak x-show="!product.image && product.customProduct !== true"
                                     ...
                                     class="object-cover rounded-lg w-16 h-16">
                                <span class="bg-primary-500 text-white text-xs font-bold rounded-full w-6 h-6 absolute -right-2 -top-2 flex items-center justify-center border-2 border-neutral-950" x-html="product.quantity"></span>
                            </div>
                            <div class="flex-1 min-w-0 flex flex-col gap-1.5">
                                ...
                                <div class="leading-tight">
                                    ...
                                    <span class="text-xs text-white/40 ml-1" x-html="product.priceFormattedSecondary" x-show="product.priceFormattedSecondary"></span>
                                </div>
                    ...
                    <div x-show="products.length === 0" class="h-full grid place-items-center text-center text-white/30 py-10">
                        <div>
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-white/50" x-html="totalQuantity() + ' artikelen'">0 artikelen</span>
                            </div>
                            <span class="font-bold text-lg" x-html="subTotal"></span>
                        </div>

                        {{-- Kortingscodes --}}
                        <div x-show="appliedDiscountCodes.length > 0" x-cloak class="space-y-1 border-t border-white/10 pt-2">
                            <template x-for="code in appliedDiscountCodes" :key="code.code">
                                <div class="text-sm flex justify-between items-center gap-2">
                                    <span class="flex items-center gap-2 truncate text-white/70">
                                        <button type="button" @click="$wire.removeDiscountCode(code.code)" class="text-red-400 hover:text-red-300 shrink-0" title="Kortingscode verwijderen">
                            ...
                            <div class="text-sm flex justify-between items-center font-semibold" x-show="appliedDiscountCodes.length > 1">
                                <span class="text-white/70">Kortingen totaal</span>
                                <span class="text-primary-400" x-html="discount"></span>
                            </div>
                        </div>
                        <div x-show="activeDiscountCode && appliedDiscountCodes.length === 0" x-cloak class="text-sm flex justify-between items-center border-t border-white/10 pt-2">
                            <span class="text-white/70">Korting</span>
                            <span class="font-semibold text-primary-400" x-html="discount"></span>
                        </div>

                        {{-- Cadeaubonnen --}}
                        <div x-show="giftCards.length > 0" x-cloak class="space-y-1 border-t border-white/10 pt-2">
                            <template x-for="card in giftCards" :key="card.code">
                                <div class="text-sm flex justify-between items-center gap-2">
                                    <span class="flex items-center gap-2 truncate text-white/70">
                                        <button type="button" @click="$wire.removeGiftCardCode(card.code)" class="text-red-400 hover:text-red-300 shrink-0" title="Cadeaubon verwijderen">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg>
                                        </button>
                            ...
                            <div class="text-sm flex justify-between items-center font-semibold" x-show="giftCards.length > 1">
                                <span class="text-white/70">Cadeaubonnen totaal</span>
                                <span class="text-primary-400">- <span x-html="giftCardsTotal"></span></span>
                            </div>
                        </div>

                        {{-- Verzendkosten --}}
                        <div x-show="shippingMethodId" x-cloak class="text-sm flex justify-between items-center border-t border-white/10 pt-2">
                            <span class="text-white/70">Verzendkosten</span>
                        ...
                        <div class="border-t border-white/10 pt-2 space-y-1">
                            <template x-for="value,percentage in vatPercentages" x-show="vatPercentages.length">
                                <div class="text-sm flex justify-between items-center text-white/70">
                            ...
                            <div x-show="vatPercentages.length == 0" class="text-sm flex justify-between items-center text-white/70">
                        ...
                        <template x-if="isExVat">
                            <div class="flex items-center justify-between border-t border-white/10 pt-2 font-bold">
