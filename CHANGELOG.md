...
### Added
- **Facturen en pakbonnen zijn afgeschermd.** `Order::createNormalInvoice()` en verwanten schrijven prive weg (`put(..., 'private')`), want de `dashed`-schijf is op S3 `visibility: public` en elk object was op naam rechtstreeks te openen. De downloadroute is nu de enige weg naar het bestand; die werkt op de orderhash (32 willekeurige tekens) en zit achter de verzoeklimiet `dashed-order-pages`, zodat een klant zonder account de factuur uit zijn bevestigingsmail blijft openen, ook uit oude mails. `downloadInvoiceUrl()`/`downloadPackingslipUrl()` geven daarnaast een ondertekende link zonder vervaldatum; met `dashed-ecommerce-core.invoices.require_signature` (`DASHED_INVOICE_REQUIRE_SIGNATURE`, standaard uit) eist de route die handtekening ook, tenzij de klant van de bestelling of een beheerder is ingelogd (`InvoiceAccess`). Een migratie zet `PrivatizeInvoicesJob` in de wachtrij die bestaande bestanden op prive zet; `dashed:privatize-invoices` doet hetzelfde met de hand.
- **Verzoeklimieten op de front-end-routes.** `throttle:dashed-order-pages` op factuur, pakbon, proforma, restbetaling en retourstatus; `throttle:dashed-cart` op toevoegen, bijwerken, verwijderen en herstellen; `throttle:dashed-discount-code` op de kortingscode. De aantallen staan in dashed-core bij Instellingen, Beveiliging.
- **robots.txt.** De bestel-, winkelwagen- en downloadroutes worden aangemeld bij `RobotsTxtBuilder` van dashed-core.
- **Orderwaarde voor verzendmethodes bijsturen.** `ShoppingCart::resolveShippingOrderValueUsing(?Closure $resolver)` registreert een closure die de orderwaarde krijgt waartegen de minimale en maximale orderwaarde van verzendmethodes getoetst worden (plus de cartregels) en de waarde teruggeeft die gebruikt moet worden. Zo kan een webshop bijvoorbeeld een groot product met een laag bedrag toch als "normale" bestelling laten verzenden in plaats van als brievenbuspakje. Geldt voor de wagen-gebaseerde aanroep in zowel de checkout als de hervalidatie bij het afrekenen; bij een expliciet meegegeven totaal (proforma) niet.
- **Bestelling wijzigen.** Nieuwe actie "Bestelling wijzigen" op de orderpagina met een eigen wijzigscherm (`Pages\ModifyOrder`), alleen beschikbaar zolang `Order::isModifiable()` true teruggeeft (diezelfde guard bewaakt zowel de knop als de pagina zelf). Het scherm toont eerst een bevestigingsstap die de wijziging samenvat (oude/nieuwe regels, bij- of terug te betalen bedrag) voordat er iets wordt weggeschreven. Onbetaalde orders zonder echt factuurnummer worden in plaats aangepast; betaalde of gefactureerde orders krijgen een vervangende order waarin het al betaalde bedrag verrekend wordt, zodat er alleen een meerprijs openstaat. De oude order wordt geannuleerd of gecrediteerd en gekoppeld via de nieuwe kolom `replaced_by_order_id`. Domeinlogica in `Classes\OrderModificationService`, totalen in `Classes\OrderTotalsCalculator`.
  ...
  - `OrderModifiedMail` naar de klant met een betaallink bij een meerprijs of een terugstortmelding bij te veel betaald; volgt het bestaande `RegistersEmailTemplate`-patroon, dus per site editbaar in de CMS en wordt per locale van de order geresolved. De nieuwe factuur van de (gewijzigde of vervangende) bestelling gaat als PDF mee als bijlage — de service slaat `SendInvoiceJob` bewust over, dus zonder die bijlage zou de klant de factuur nooit krijgen. Een ontbrekende PDF houdt de mail niet tegen.
  - `Order::overpaidAmount()` plus de actie "Terugstorting registreren" om een teveel betaald bedrag als afgehandeld te markeren. Het bedragveld is begrensd op minimaal 0,01 en maximaal het teveel betaalde bedrag, zodat een typefout een validatiemelding geeft in plaats van een onbehandelde exception.
  - De productzoeker in het wijzigscherm is gescoped op de site van de bestelling, zodat er op een multi-site-installatie geen product van een andere webshop op een order kan belanden.
  - Een procentuele kortingscode wordt bij een wijziging herrekend over het nieuwe subtotaal (zelfde regels als de winkelwagen: per regel getoetst op `valid_for`, per stuk afgerond, niet over verzend- en betaalkosten). Codes met een vast bedrag blijven bevroren op het bij het afrekenen afgesproken bedrag. De bevestigingsstap toont het toegepaste kortingsbedrag.
  - Niet-bewerkbare regelvelden (`sku`, `discount`, `is_pre_order`) blijven staan zolang een regel over hetzelfde product gaat, maar verhuizen niet mee naar een ander product. Zet de beheerder in een bestaande regel een ander product, dan komt de sku van dát product (zelfde conventie als de winkelwagen), en een nieuw toegevoegde productregel krijgt zijn sku uit het product in plaats van leeg te blijven. Zonder dat onderscheid hield een omgezette regel de sku van zijn voorganger — bij een omgezette verzendkostenregel boekte een echt product zich dan als verzendomzet — en viel een order met een lege sku uit de verkochte-aantallen (`sku NOT IN (...)` is bij NULL nooit waar). De bevestigingsstap bepaalt de sku met exact dezelfde methode, zodat het getoonde kortingsbedrag niet kan afwijken van wat er weggeschreven wordt.
  - Wordt de korting afgetopt omdat er minder op de bestelling overblijft dan de korting, dan komt dat in het orderlogboek te staan (`order.discount.capped`, met het oorspronkelijke bedrag, het afgetopte bedrag en het verschil) en waarschuwt de bevestigingsstap er vooraf voor. Bij een cadeaubon wordt de bon met code genoemd: dat saldo wordt bewust niet automatisch teruggeboekt, dus het moet handmatig gecorrigeerd worden.
  - De kortingsboekhouding blijft bij een wijziging per saldo onaangeraakt: `discount_amount`, `used_amount`, `reserved_amount` en `stock_used` van de gekoppelde code (ook van een cadeaubon) zijn na afloop gelijk aan ervoor, in beide takken en bij elke status. De vervangende order erft de code en verbruikt hem dus niet een tweede keer.
  - De vervangende order krijgt `created_at` van de eerste betaling (net als `markAsPaid()` doet), zodat omzet niet van de betaalmaand naar de maand van de wijziging verschuift.
  - `packed_at`, `invoice_send_to_customer`, `is_proforma`, `proforma_sent_at` en `cart_id` gaan niet mee naar de vervangende order.
  - `Order::markAsCancelled()` heeft een tweede parameter `$refillStock` gekregen zodat al verzonden producten niet terug in voorraad komen, en een derde `$refillDiscount` (default `true`, dus bestaand gedrag) om de kortingstellers met rust te laten wanneer de "annulering" in werkelijkheid een verplaatsing naar een vervangende order is.
  - `Order::markAsCancelledWithCredit()` heeft `$sendAdminEmail` en `$refillGiftcard` gekregen (beide default `true`, dus bestaand gedrag). Een wijziging via de credittak stuurt de beheerders daardoor geen annuleringsmail, laat de fulfillment-status van de oude order ongemoeid (geen `FulfillmentStatusHandledMail` en geen inschrijving in de na-aankoop-flows) en stort de cadeaubon niet terug.
- **Klantgegevens meekopiëren naar de kassa.** De actie "Kopiëren naar kassa" op de orderpagina heeft een vinkje "Klantgegevens meekopiëren" gekregen dat standaard aan staat. Staat het aan, dan komen naam, e-mail, telefoonnummer, adres, bedrijfsnaam, btw-nummer en de factuurgegevens van de bestelling mee op de kassa-winkelwagen (de omgekeerde afbeelding van `ConceptOrderService::saveAsConcept()`), zodat de kassamedewerker ze niet hoeft over te typen. De gekoppelde klant gaat alleen mee als de bestelling echt aan een klant hangt en niet aan de kassamedewerker zelf: `saveAsConcept()` valt voor `user_id` terug op de medewerker, en die terugval hoort niet als klant op een nieuwe verkoop te belanden. `ConceptOrderService::copyIntoCart()` heeft er een derde parameter `$copyCustomerDetails` voor (default `false`, dus bestaand gedrag voor bestaande aanroepen).
...
- **Cart-badge crashte op een vervalst Livewire-verzoek.** `CartCount` had `$cartCount` als publieke eigenschap en echode die rechtstreeks in de view. Livewire laat een browser elke publieke eigenschap zetten via de `updates` in het verzoek, dus een array daarin gaf `htmlspecialchars(): Argument #1 ($string) must be of type string, array given` op `livewire/update` (ruim duizend keer sinds maart), en een getal werd zonder meer getoond. Het aantal is nu afgeleide staat: `render()` leest het per keer uit de winkelwagen en er is geen publieke `$cartCount` meer om te zetten. De view krijgt `$cartCount` nog steeds mee, dus thema-overrides van `cart/cart-count.blade.php` hoeven niet aangepast.
