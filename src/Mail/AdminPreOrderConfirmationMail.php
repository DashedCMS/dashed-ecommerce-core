...
class AdminPreOrderConfirmationMail extends Mailable implements RegistersEmailTemplate, SendsToTelegram
{
    ...
    {
        return 'Pre-order bevestiging (beheerder)';
    }

    public static function emailTemplateDescription(): ?string
    {
        return 'Verzonden naar de beheerder zodra een pre-order is geplaatst.';
    }
    ...
    {
        return 'Nieuwe pre-order ontvangen #:orderId:';
    }

    public static function defaultBlocks(): array
    {
        return [
            ['type' => 'heading', 'data' => ['text' => 'Nieuwe pre-order #:orderId:', 'level' => 'h1']],
            ['type' => 'text', 'data' => ['body' => '<p>Er is een nieuwe pre-order geplaatst.</p>']],
    ...
    public function build()
    {
        ...
        if ($templateHtml !== null) {
            ...
            $view = view()->exists(config('dashed-core.site_theme', 'dashed') . '.emails.admin-confirm-pre-order')
                ? config('dashed-core.site_theme', 'dashed') . '.emails.admin-confirm-pre-order'
                : 'dashed-ecommerce-core::emails.admin-confirm-pre-order';
