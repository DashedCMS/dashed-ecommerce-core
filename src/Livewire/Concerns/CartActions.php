...
trait CartActions
{
    ...
    {
        $product = $productId ? Product::find($productId) : ($this->product ?? null);

        cartHelper()->setCartType($this->cartType);

        // Als jij in je component shipping method/zone instelt, zorg dan dat je dit ook doet:
        // cartHelper()->setShippingMethod($this->shippingMethod?->id);
        // cartHelper()->setShippingZone($this->shippingMethod?->shipping_zone_id);
        ...
        // MERGE: zelfde product + zelfde options + zelfde hidden options
        ...
        foreach ($cartItems as $cartItem) {
            if (
                ...
                $newQuantity = (int) $cartItem->qty + (int) $this->quantity;
